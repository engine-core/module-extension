...
$column = [
    ...
    [
        ...
        'template'       => '{install} {enable} {uninstall} {tips}',
        'visibleButtons' => [
            'install'   => function ($infoInstance, $key, $index) {
                /** @var ExtensionInfo $infoInstance */
                return $infoInstance->getCanInstall();
            },
            'enable'    => function ($infoInstance, $key, $index) {
                /** @var ExtensionInfo $infoInstance */
                return !$infoInstance->getCanInstall() && ($infoInstance->getIsEnable() == EnableEnum::DISABLE);
            },
            ...
            'tips'      => function ($infoInstance, $key, $index) {
                /** @var ExtensionInfo $infoInstance */
                return !$infoInstance->getCanUninstall() && !$infoInstance->getCanInstall() && $infoInstance->getIsEnable();
            },
        ],
        'buttons'        => [
            ...
            'enable'    => function ($url, $model, $key) use ($app) {
                return Html::a(Yii::t('ec/modules/extension', 'Enable'), ['enable', 'id' => $key, 'app' => $app], [
                    'data-method'  => 'post',
                    'data-confirm' => Yii::t('ec/modules/extension', 'Are you sure you want to use the current theme?'),
                ]);
            },
